...
class TalkeOverView extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            talks: [
    ...
    render() {
        ...
        const dataByDay = talks.filter((item) => item.day === selectedDay);
        const list = dataByDay.map((item) => <TalkItem key={item.id} {...item} />)
        return (
            <div className='talkOverview'>
                <Navigation days={days} selectedDay={selectedDay} 
                changeSelectedDay={(day) => this.setState({selectedDay: day})} />
